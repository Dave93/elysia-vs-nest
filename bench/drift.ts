...
try {
  for (const c of CS) {
    ...
    const rps: number[] = [], cpu: number[] = [], other: number[] = [];
    for (let i = 0; i < 3; i++) { const s = startSampler(srv.pid); const y = startSysSampler(srv.pid); rps.push(await bomb(bombardierArgs(c), "30s")); cpu.push(mean(s.stop().map((x) => x.cpu))); other.push(mean(y.stop())); }
    const morning = R.runs.find((r: any) => r.config === "A" && r.case === c.name && r.concurrency === 100).median.rps;
    out.cases[c.name] = { rps, median: median(rps), meanCpu: mean(cpu), otherCpu: mean(other), morningRps: morning, deltaPct: (median(rps) / morning - 1) * 100 };
